...
```json "start": "node backend/server.js",
...
```js const express = require('express') // import express
...
```js const express = require('express') // import express const dotenv = require('dotenv').config() // import dotenv to use .env file for environment variables
const port = process.env.PORT || 5000 // set port to 5000 or whatever is in .env file
